...
public class ShippedEventAdapter implements EntryAdapter<OrderEvents.OrderShipped,BaseEntry.TextEntry> {
    ...
    @Override
    public TextEntry toEntry(final OrderEvents.OrderShipped source, final int version, final String id, final Metadata metadata) {
      final String serialization = JsonSerialization.serialized(source);
      return new BaseEntry.TextEntry(id, OrderEvents.OrderShipped.class, 1, serialization, version, metadata);
    }
}
